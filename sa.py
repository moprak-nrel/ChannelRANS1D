...
Re_tau_round = 5200
Re_tau_table = {180:182.088,550:543.496,1000:1000.512,2000:1994.756,5200:5185.897,10000:10000}
...
def get_dUdt(U,dyU,dyyU,nu_tilde):
    ...
    res =  1 + (1./Re_tau+nuT)*dyyU + get_y_der(ntck)*dyU
    return res

def get_dUdt_fd(U,dyU,dyyU,nu_tilde):
    nuT = get_nuT(nu_tilde)
    res =  1 + (1./Re_tau+nuT)*dyyU + get_dy_fd(nuT)*dyU
    return res
...
def get_Stilde( dyU, nu_tilde):
    ...
    S_tilde[1:] = dyU[1:] +( - nu_tilde[1:]**2 / ( nu + nuT[1:] ) + nu_tilde[1:] )/(K*Y[1:])**2 
    return S_tilde

def get_Pnu(dyU,nu_tilde):
    return cb1 * (dyU + ( - nu_tilde**2/(nu+nu_tilde) + nu_tilde)/((K*Y)**2))*nu_tilde # S*nu_tilde
...
def get_dnudt(U,dyU,nu_tilde,dynu,dyynu):
    res =  get_Pnu(dyU,nu_tilde) - get_Enu(dyU,nu_tilde) + 1./sigmav * ( (nu+nu_tilde)*dyynu + (1+cb2)*dynu**2)
